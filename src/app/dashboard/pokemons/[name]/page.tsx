...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        ...
        const { id, name } = await getInfoPokemon(params.name);
...
const getInfoPokemon = async (name: string): Promise<Pokemon> => {

    try {

        const infoPokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
            cache: 'force-cache'
        })
            .then(res => res.json());
        return infoPokemon;
...
export default async function NamePokemonPage({ params }: Props) {
    ...
    const pokemon = await getInfoPokemon(params.name);
